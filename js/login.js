...
if (typeof(Storage) !== "undefined") {
  ...
  //each user object has username, current number of bees to give, and their avatar properties
  var sampleUsers = [{username:"Jason D", beesToGive: 10, avatar:"images/demoUser1.png"},{username:"Ashley E", beesToGive: 10, avatar:"images/demoUser2.png"},{username:"Erik H", beesToGive: 10, avatar:"images/demoUser3.png"}, {username:"Yahor N", beesToGive: 10, avatar:"images/demoUser3.png"}];
  ...
  function login() {
    ...
    var index = isAuthentic(samplePerson, sampleSlackUsers);
    //console.log(index);
    if(index[0]) {
  ...
  function isAuthentic(person, arr) {
    //will loop only until found, won't go through whole slack db
    for(var i = 0; i < arr.length ;i++) {
      if(arr[i].username.trim().toLowerCase() === person.trim().toLowerCase()) {
  ...
  function checkDB(person) {
    ...
    var index = isAuthentic(person, sampleUsers);
    console.log(index);
    if(!index[0]) {
    ...
    console.log(sampleUsers);
    storeSession(sampleUsers[index[1]]);
    goToUserProfilePage();
  };
  ...
  function storeSession(person) {
    var strPerson = JSON.stringify(person);
    console.log(person);
    console.log(strPerson);
    sessionStorage.setItem("currUser",strPerson);
  };
